...
import { HeroBackgroundAnimation } from '../components/Diagrams';
  ...
  return (
    <>
      ...
      >
        ...
        {pillars.map((p, i) => (
          <circle key={i} className={`dot-${i}`} cx="260" cy={p.y} r="4" fill="#1E1E2A"/>
        ))}
        ...
        <g className="box-r">
          ...
          <rect x="440" y="140" width="160" height="36" rx="4" fill="#2d3748"/>
          ...
          <text x="476" y="232" fontSize="8" fill="rgba(255,255,255,0.5)" letterSpacing="1">LIVE</text>
          <text x="580" y="232" fontSize="8" fill="rgba(255,255,255,0.35)" textAnchor="end" letterSpacing="1">24/7</text>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 md:gap-16 items-start">
            <div className="lg:col-span-4">
              ...
              <div className="space-y-0 border border-stone-200">
                {[
                  { label: "Delayed Lead Handling", desc: "Response times exceeding 4 hours result in 60% drop off rates.", cost: "−£75k/yr" },
                  { label: "Recruitment Friction", desc: "Competitors move faster, securing quality staff before you follow up.", cost: "−£5k/hire" },
                  { label: "Staff Overload", desc: "Senior management bogged down in manual enquiry logging.", cost: "−30 hrs/wk" }
                ].map((item, i) => (
                  <motion.div
                    key={i}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 h-full">
                <div className="bg-stone-50 border border-stone-200 md:border-r-0 p-8 md:p-10">
                  ...
                  <div className="space-y-8">
                    {[
                      { metric: "4–24 hrs", label: "Average enquiry response" },
                      { metric: "85%", label: "Of private leads lost to slow response" },
                      { metric: "3–5 days", label: "To contact a new job applicant" },
                      { metric: "40%", label: "Agency markup on every hire" },
                      { metric: "30 hrs", label: "Per week lost to admin tasks" },
                    ].map((item, i) => (
                      <motion.div
                        key={i}
      ...
      <section className="py-20 md:py-32 px-6 md:px-8 bg-white relative z-10 border-y border-stone-200">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="border border-stone-200 overflow-hidden">
            ...
            {[
              { feature: "Inquiry Handling", old: "Manual Callbacks (8 to 24 hrs)", new: "Instant Automated Triage" },
              { feature: "Staffing", old: "Reactive Agency Hiring", new: "Always on Talent Pipeline" },
              { feature: "Visibility", old: "Gut feeling & Spreadsheets", new: "Live Performance Dashboards" },
              { feature: "Cost", old: "High Recurring Admin Costs", new: "Fixed cost Efficiency Engine" },
              { feature: "Scalability", old: "Needs More Admin Hires", new: "Infinite Growth Foundation" }
            ].map((row, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0 }}
